...
map<string, int> wordFrequencies(vector<string> s) {
    map<string, int> map;
...
void checkMagazine(vector<string> magazine, vector<string> note) {
    auto magazineWords = wordFrequencies(magazine);
    for (int i = 0; i < size(note); i++) {
        if (magazineWords.find(note[i]) != magazineWords.end() && magazineWords[note[i]] > 0) {
